...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        "*** BEGIN YOUR CODE HERE ***"
        tmp = self.computationHelper(state)
        return tmp[0]
        "*** END YOUR CODE HERE ***"
    ...
    def computationHelper(self, state): 
        actions = self.getLegalActions(state)
        if len(actions) == 0:  
            return (0.0, None)
        opt, move = float("-inf"), None
        for action in actions: 
            curr = self.getQValue(state, action)
            if curr >= opt:
                opt, move = curr, action
        return (opt, move)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for ft in feats: 
            self.weights[ft] += self.alpha * diff * feats[ft]
        "*** END YOUR CODE HERE ***"
